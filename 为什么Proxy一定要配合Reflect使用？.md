...
- 首先，之前我们提到过在 Proxy 中 get 陷阱的 receiver 不仅仅会表示代理对象本身同时也还有可能表示继承于代理对象的对象，具体需要区别于调用方。这里显然它是指向继承与代理对象的 obj 。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz/H8M5QJDxMHrrfiavfNj7YibfTK4U4010DzZFibzfEZfXnBMvwJgSEJawCChnm5la9ZhdzXune6pTiand6QfGKxicwbA/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)image.png

# 总结

相信看到这里大家都已经明白了，为什么Proxy一定要配合Reflect使用。恰恰是为什么触发代理对象的劫持时，要保证正确的 this 上下文指向。
